...
import { useCreateProject, useProjectData, useProjectsData, useUpdateProject } from '@/hooks/Projects.hooks';
...
const UpdateProject = ({ params }: Props) => {
    ...
    const { updateProject, success, loading: updateLoading, error: updateError } = useUpdateProject(token)
    const { uploadImage, data: ImageUrl, loading: imageLoading, error: imageError } = useUploadImage(token)
    const { loading, project, error } = useProjectData(token, params?.projectId, triggerRefetch)
            ...
            <div className='w-full min-h-[70vh]'>
                <div>
                    <div className='w-full flex justify-end items-center'>
                        <Button onClick={() => router.push(`/admin/programs-and-projects`)} className='bg-white text-black border-2 border-[#dcdee6] hover:bg-white hover:text-black'>Go back</Button>
                    </div>
                    <Form {...form}>
