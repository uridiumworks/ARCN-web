...
const UpdateProgram = ({ params }: Props) => {
    ...
    const { updateProgram, success, loading: updateLoading, error: updateError } = useUpdateProgram(token)
    const { uploadImage, data: ImageUrl, loading: imageLoading, error: imageError } = useUploadImage(token)
    const { loading, program, error } = useProgramData(token, params?.programId, triggerRefetch)
