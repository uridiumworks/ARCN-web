...
const SupervisionReportAction = ({supervisionReportData}: any) => {
  const [dialog, setDialog] = useState(0);
  const router = useRouter();
  return (
    <Dialog>
      {dialog === 1 && <DeleteAction id={supervisionReportData?.supervisionReportId}/>}
      <DropdownMenu modal={false}>
        ...
        >
          <DropdownMenuItem
            ...
            onClick={() => router.push(`/admin/training/supervision-report/${supervisionReportData?.supervisionReportId}`)}
          >
...
export default SupervisionReportAction