...
const formSchema = z.object({
  ...
  bannerUrl: z
    .string()
    .min(1, { message: "uploaded Banner must be provided." }),
  ...
  authorEmail: z
   ...
   publishOn: z.string().min(3, { message: "Publish Date must be provided" }),
});
...
const SupervisionReportForm = ({ setCreateSupervisionReport }: Props) => {
 ...
 const {isCreating,createTrainingSupervsionReport} = useTrainingSupervsionReportContext()
  const {
    uploadImage,
    data: ImageUrl,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
   try {
    console.log(values);
    await createTrainingSupervsionReport(values);
    setCreateSupervisionReport(false)
   } catch (error) {
  ...
  return (
    <div>
      <div className="w-full flex justify-end items-center">
        <Button
          onClick={() => setCreateSupervisionReport(false)}
