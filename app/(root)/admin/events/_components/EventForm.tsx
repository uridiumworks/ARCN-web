...
const EventForm = ({ setCreateNewEvent }: Props) => {
  ...
  // Add time validation for same-day events
  useEffect(() => {
    const startDate = form.watch("eventStartDate")
    const endDate = form.watch("eventEndDate")
    const startTime = form.watch("eventStartTime")
    const endTime = form.watch("eventEndTime")

    // Only validate times if both are provided and it's a same-day event
    if (startDate && endDate && startTime && endTime && startDate === endDate) {
      if (endTime < startTime) {
        ...
        form.clearErrors("eventEndTime")
      }
    }
  }, [
    form.watch("eventStartDate"),
    form.watch("eventEndDate"),
          ...
          <div className="w-full flex flex-col gap-2 md:flex-row md:justify-start md:gap-5 mt-5">
            ...
            <div className="w-full mt-3 md:mt-0 md:w-[30%] min-h-[70vh] border-[1px] border-[#dcdee6] py-5 px-3">
              ...
              <div className="grid grid-cols-1 gap-6 mt-5">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} type="time" autoComplete="new-password" className="bg-inherit outline-none" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                      ...
                      <FormControl>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <SelectTrigger className="w-full bg-inherit">
                            <SelectValue placeholder={field.value || "Select Option"}>
                ...
                >
                  {isCreating ? <ButtonSpinner /> : <span className="text-[14px] font-noraml">Post</span>}
                </Button>
              </div>
            </div>
          </div>
