...
const ResetPassword = () => {
    ...
    const [currentPasswordStrengthText, setCurrentPasswordStrengthText] = useState<string>("");
    ...
    useEffect(() => {
        ...
        if (newPassword) { // Only check strength for newPassword
        ...
        if (currentPassword) { // Only check strength for newPassword
    ...
    return (
        <div className="px-10 py-3">
            <div>
                <span className='text-xs text-[#4b5563]'>we will send a reset link to the email attached to this account</span>
            </div>
            <div className="mt-2">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} autoComplete="current-password">
                        ...
                        <Button className="w-full bg-[#30a85f] mt-5 text-[#fff] text-center border-2 border-[#dcdee6] flex justify-center items-center hover:bg-[#30a85f] hover:text-[#fff]"><span className="text-[14px] font-noraml">Change Password</span></Button>
                    </form>
                </Form>
            </div>
        </div>
