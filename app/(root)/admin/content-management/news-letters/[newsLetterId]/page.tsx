...
import { useNewsLetterData, useUpdateNewsLetter } from '@/hooks/NewsLetters.hooks';
...
const UpdateNewsLetters = ({ params }: Props) => {
    const router = useRouter();
    const docImgRef = useRef<HTMLInputElement | null>(null);
    const [imageName, setImageName] = useState<string>("")
    const [token, setToken] = useState<string | null>(null)
    const [triggerRefetch, setTriggerRefetch] = useState<boolean>(false)
    const { updateNewsLetter, success, loading: updateLoading, error: updateError } = useUpdateNewsLetter(token)
    const { loading, newsLetter, error } = useNewsLetterData(token, params?.newsLetterId, triggerRefetch)
    ...
    <div className="w-full min-h-[70vh]">
      <div>
        <div className="w-full flex justify-end items-center">
          <Button
            onClick={() => router.push(`/admin/content-management/news-letters`)}
...
export default UpdateNewsLetters