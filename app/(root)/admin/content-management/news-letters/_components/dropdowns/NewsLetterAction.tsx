...
const NewsLetterAction = ({newsLetterData}: any) => {
  const [dialog, setDialog] = useState(0);
  const router = useRouter();
  return (
    <Dialog>
      {dialog === 1 && <DeleteAction id={newsLetterData?.newsLetterId}/>}
      <DropdownMenu modal={false}>
        ...
        >
          <DropdownMenuItem
            ...
            onClick={() => router.push(`/admin/content-management/news-letters/${newsLetterData?.newsLetterId}`)}
          >
          ...
          <DropdownMenuItem
            className="flex justify-start gap-3 items-center cursor-pointer"
            onClick={() => router.push(`/admin/content-management/journals/${newsLetterData?.newsLetterId}/view-Journal`)}
          >
