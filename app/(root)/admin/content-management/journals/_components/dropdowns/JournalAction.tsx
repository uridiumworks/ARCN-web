...
const JournalAction = ({journalData}: any) => {
  const [dialog, setDialog] = useState(0);
  const router = useRouter();
  return (
    <Dialog>
      {dialog === 1 && <DeleteAction id={journalData?.journalsId}/>}
      <DropdownMenu modal={false}>
        ...
        >
          <DropdownMenuItem
            ...
            onClick={() => router.push(`/admin/content-management/journals/${journalData?.journalsId}`)}
          >
          ...
          <DropdownMenuItem
            className="flex justify-start gap-3 items-center cursor-pointer"
            onClick={() => router.push(`/admin/content-management/journals/${journalData?.journalsId}/view-Journal`)}
          >
